...
*/
function wp_series_nav($series_ID, $next = TRUE, $customtext = 'deprecated', $display = FALSE, $calc = false, $full = false) {
	global $post, $orgseries;
	...
	if($full){
		$posts_in_series = get_series_order($series_posts, $empty, $series_ID);
		$no_of_posts = count($posts_in_series);
		$result .= '<style>.navi{width: calc((100% - 108px) / ' . $no_of_posts . ');</style>';
		foreach ($posts_in_series as $seriespost) {
			if ((int) $seriespost['part'] == 1) {
				$title = get_the_title($seriespost['id']);
				$link = get_permalink($seriespost['id']);
				$result .= '<a style="float: left; margin-right: 10px;" class="firstPost" href="' . $link . '" title="' . $title . '">&lt;&lt;</a>';
			}
			if (($cur_part - (int) $seriespost['part']) === 1) {
				$title = get_the_title($seriespost['id']);
				$link = get_permalink($seriespost['id']);
				$result .= '<a style="float: left;" class="prev" href="' . $link . '" title="' . $title . '">&lt;</a>';
			}
			$title = $seriespost['part'];
			$link = get_permalink($seriespost['id']);
			if($seriespost['id'] == $cur_id){
				$result .= '<a class="currentSeriesPost navi" href="#" title="' . $title . '">' . $title . '</a>';
			} else {
				$result .= '<a class="navi" href="' . $link . '" title="' . $title . '">' . $title . '</a>';
			}
			if((int) $seriespost['part'] == (int) $no_of_posts) {
				$title = get_the_title($seriespost['id']);
				$link = get_permalink($seriespost['id']);
				$result .= '<a style="float: right; margin-left: 10px;" class="lastPost" href="' . $link . '" title="' . $title . '">&gt;&gt;</a>';
			}
			if ( ( (int) $seriespost['part'] - $cur_part) === 1) {
				$title = get_the_title($seriespost['id']);
				$link = get_permalink($seriespost['id']);
				$result .= '<a style="float: right;" class="next" href="' . $link . '" title="' . $title . '">&gt;</a>';
			}
		}
